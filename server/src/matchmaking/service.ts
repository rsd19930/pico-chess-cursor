...
import { createBot } from '../pico/simpleBot';
import type { GameService } from '../pico/gameService';
import { allLegalMoves, kingSquare, isSquareAttacked, opposite } from '../pico/rules';
...
type GameRoom = {
  ...
  black: string; // socketId or 'BOT'
};

export function createMatchmakingService(io: Server, gameService: GameService, config: { botJoinTimeoutMs: number }) {
  const waitingQueue: WaitingPlayer[] = [];
  const socketIdToRoomId = new Map<string, string>();
  const rooms = new Map<string, GameRoom>();
  const lastTick = new Map<string, number>();

  function cleanupWaiting(socketId: string) {
  ...
  function requestMatch(socket: Socket, name: string) {
    ...
    const waiting: WaitingPlayer = { socketId: socket.id, name };
    waiting.timer = setTimeout(() => {
      const idx = waitingQueue.findIndex(w => w.socketId === socket.id);
      if (idx === -1) return; // already matched or removed
  ...
  function handleMove(socket: Socket, payload: { gameId: string; from?: string; to?: string; drop?: { piece: string; square: string } }) {
    ...
    if (gameAfter?.bot && ((gameAfter.state.turn === 'black' && room.black === 'BOT') || (gameAfter.state.turn === 'white' && room.white === 'BOT'))) {
      ...
      setTimeout(() => {
        ...
        if (!gameNow || gameNow.state.turn !== botColor) return;
        ...
        if (botResult) {
          ...
          if (botResult.gameOver) io.to(gameId).emit('game_over', botResult.gameOver);
          else lastTick.set(gameId, Date.now());
        }
      }, delay);
    }
  }

  function handleResign(socket: Socket, payload: { gameId: string }) {
    const room = rooms.get(payload.gameId);
    if (!room) return;
    const color = socket.id === room.white ? 'white' : socket.id === room.black ? 'black' : undefined;
    if (!color) return;
    const result = gameService.resign(payload.gameId, color);
    if (result) io.to(payload.gameId).emit('game_over', result);
  }
  ...
  function handleDisconnect(socket: Socket) {
    ...
    if (result) io.to(gameId).emit('game_over', result);
    rooms.delete(gameId);
  ...
  setInterval(() => {
    ...
    for (const [gameId, room] of rooms) {
      ...
      if (!game || game.state.winner) continue;
      ...
      if (elapsed >= 200) io.to(gameId).emit('state', game.state);
      if (remaining === 0) {
        ...
        const ksq = kingSquare(game.state.board, game.state.turn);
        game.state.check = ksq ? isSquareAttacked(game.state.board, ksq, opposite(game.state.turn)) : false;
        game.state.legal = allLegalMoves(game.state, game.state.turn);
        // Reset per-turn clock for the new player
        const newKey = game.state.turn === 'white' ? 'whiteMs' : 'blackMs';
        game.state.clocks[newKey] = 30_000;
        // reset the elapsed reference
        lastTick.set(gameId, Date.now());
        // restore current turn's clock if needed
        const otherKey = game.state.turn === 'white' ? 'blackMs' : 'whiteMs';
        if (game.state.clocks[otherKey] < 0) game.state.clocks[otherKey] = 0;
        ...
        if (game.bot && ((game.state.turn === 'black' && room.black === 'BOT') || (game.state.turn === 'white' && room.white === 'BOT'))) {
          ...
          setTimeout(() => {
            const g = gameService.getGame(gameId);
            if (!g || g.state.turn !== botTurn) return;
